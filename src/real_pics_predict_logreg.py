...
#Extract the images from the folders and makes them into 224x224sizes for faster
...
X_test_pics=np.asarray(X_test_pics)
y_train=np.asarray(y_train)
y_test_pics=np.asarray(y_test_pics)
...
ax[0].title.set_text(f'Solution=1, predicted={y_pred[0]}')
ax[1].imshow(real_pic_mask_list[1])
ax[1].title.set_text(f'Solution=1, predicted={y_pred[1]}')
ax[2].imshow(real_pic_nomask_list[1])
ax[2].title.set_text(f'Solution=0, predicted={y_pred[2]}')
ax[3].imshow(real_pic_nomask_list[0])
ax[3].title.set_text(f'Solution=0, predicted={y_pred[3]}')
